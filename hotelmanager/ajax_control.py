...
from hotelmanager.models import Staff
...
def search_room(request):
    ...
    for i in range(6):
        # num = 0
        # cursor.callproc('count_room_num', (datein, dateout, lv[i], num))
...
def insert_check(request):
    ...
    if name == "" or phone == "" or idnum == "" or ctime == "" or ltime == "" or room_id == "":
        ...
        checkinfo = Checkinfo.objects.filter(check_id=check_id).update(check_name=name, check_phone=phone,
                                                                       check_idnum=idnum, check_leavetime=ltime,
                                                                       check_checkintime=ctime, check_statu=statu,
                                                                       room_id=room_id)
        info["info"] = "1"
...
def delete_room(request):
    ...
    if room_id == "" or len(room_id) != 3:
        ...
        try:
            ...
            cursor = connection.cursor()
            sql = "select count(*) from checkinfo WHERE check_statu = 'pre' AND room_id= '"+room_id+"';"
            result = cursor.execute(sql)
            row = cursor.fetchone()
            if row[0] == 0:
                room.delete()
...
def check_staff(request):
    ...
    try:
        staff = Staff.objects.get(sta_id=staff_id)
    except:
        print("in except")
        json_data["info"] = 0
    else:
        pwd = staff.sta_passward
        if staff_pwd == pwd:
            if staff.sta_authority == "high":
                json_data["info"] = 1
            else:
                json_data["info"] = 2
        else:
...
def add_staff(request):
    ...
    staff_id = request.POST["staff_id"]
    staff_pwd = request.POST["staff_pwd"]
    staff_auth = request.POST["staff_auth"]

    if staff_id == "" or staff_pwd == "":
        json_data["info"] = 0
    try:
        staff = Staff.objects.get(sta_id=staff_id)
    except:
        sta = Staff(sta_id=staff_id, sta_authority=staff_auth, sta_passward=staff_pwd)
        sta.save()
        json_data["info"] = 1
    else:
        json_data["info"] = 2
